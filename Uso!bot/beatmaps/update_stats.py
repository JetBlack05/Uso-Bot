...
def update_stats(discordId, conn, api, scores = 20, osuId = 0):
	...
	cursor.execute("UPDATE users SET accuracy_average = ?, pp_average = ?, NoMod_average = ?, HR_average = ?, HD_average = ?, DT_average = ?, DTHD_average = ?, DTHR_average = ?, HRHD_average = ?, DTHDHR_average = ?  WHERE DiscordId = ?", [round(acc_average, 2), round(pp_average, 2), mods_perference[0], mods_perference[1], mods_perference[2], mods_perference[3], mods_perference[4], mods_perference[5], mods_perference[6], mods_perference[7], int(discordId)])
	conn.commit()

	return round(acc_average, 2), round(pp_average, 2), mods_perference

def update_all_stats(cursor):
